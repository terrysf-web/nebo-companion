...
`morning-nas/` 폴더의 브리핑 서비스는 매일 아침 그날의 브리핑을 텍스트로 만듭니다.
그 텍스트를 Nebo 페이지에 붙여넣고, 구분선(`---`) 아래에 손으로 그날 계획을 쓴 다음
페이지 전체를 Companion에 공유하면 됩니다.

구분선 위의 참고 내용(오늘 일정, 안 읽은 메일 등)은 이미 캘린더에 있으므로 저장하지
않습니다. 아래에 쓴 것만 일정·할 일·알림으로 분류합니다. 시간 눈금으로 넣은 `06`,
`07` 같은 숫자만 있는 줄도 건너뜁니다. 구분선이 없으면 예전처럼 모든 줄을 읽습니다.
